...
USE [Admin] -- Specify the database in which the objects will be created.
...
SET @BackupDirectory     = N'\\ipoff-nas-1\database_backups\az3-dev-sql-1' -- Specify the backup root directory.
SET @CleanupTime         = 72           -- Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
SET @OutputFileDirectory = N'\\ipoff-nas-1\database_backups\az3-dev-sql-1'         -- Specify the output file directory. If no directory is specified, then the SQL Server error log directory is used.
