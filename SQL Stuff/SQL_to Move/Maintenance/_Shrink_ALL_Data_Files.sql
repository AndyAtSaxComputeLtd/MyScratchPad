...
----list the big ones
--select *,TotalSize_MB-Space_Used_MB as RemainingMB , cast((Space_Used_MB+1.00)/TotalSize_MB*100 as Int) from ##Fdetails
--where (Space_Used_MB+1.00)/TotalSize_MB*100 >20 and TotalSize_MB-Space_Used_MB > 1000
--order by TotalSize_MB-Space_Used_MB desc

Insert INTO ##FDetails2(Dbname,Filename, SQLRUN ) 
	select DBname, Filename, 'use ['+dbname+'];DBCC SHRINKdatabase (N'''+dbname+''',20)'
	from ##Fdetails 
	WHERE type = 'Rows' and DBname not in ('msdb','tempdb','master','model','distribution')
		and (Space_Used_MB+1.00)/TotalSize_MB*100 >20 and TotalSize_MB-Space_Used_MB > 1000
