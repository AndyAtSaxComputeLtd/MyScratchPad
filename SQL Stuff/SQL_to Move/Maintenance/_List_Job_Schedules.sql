...
, CASE s.freq_type 
     ...
     WHEN  8 THEN ''Every '' 
        ...
        + '' weeks(s) on '' 
        + dba.dbo.fn_freq_interval_desc(s.freq_interval)					  
...
, CASE s.freq_subday_type 
     WHEN 1 THEN ''Occurs once at '' 
        + dba.dbo.fn_Time2Str(s.active_start_time) 
     WHEN 2 THEN ''Occurs every '' 
        + convert(varchar,s.freq_subday_interval) 
        + '' Seconds(s) Starting at '' 
        + dba.dbo.fn_Time2Str(s.active_start_time) 
        + '' ending at '' 
        + dba.dbo.fn_Time2Str(s.active_end_time) 
     ...
     WHEN 8 THEN ''Occurs every '' 
        ...
        + dba.dbo.fn_Time2Str(s.active_start_time) 
        + '' ending at '' 
        + dba.dbo.fn_Time2Str(s.active_end_time) END AS Frequency
, CASE WHEN s.freq_type =  1 THEN ''On date: '' 
          + dba.dbo.fn_Date2Str(s.active_start_date) 
          + '' At time: '' 
          + dba.dbo.fn_Time2Str(s.active_start_time)
       WHEN s.freq_type < 64 THEN ''Start date: '' 
          + dba.dbo.fn_Date2Str(s.active_start_date) 
          + '' end date: '' 
          + dba.dbo.fn_Date2Str(s.active_end_date) END as Duration
, dba.dbo.fn_Date2Str(xp.next_run_date) + '' '' 
    + dba.dbo.fn_Time2Str(xp.next_run_time) AS Next_Run_Date
...
truncate table dba.dbo.tmpJobSchedules
insert into dba.dbo.tmpJobSchedules(JobName, Category, Enabled, Scheduled, Description, Occurs, Occurs_detail, Frequency, Duration, Next_Run_Date)
EXEC(@sql)

select * from dba.dbo.tmpJobSchedules
where 
Jobname in ('C5AD74CC-D72B-42BE-B796-3C8FC5955FCE'
,'50FF4197-93CA-434C-923C-A9D55B19E626')
